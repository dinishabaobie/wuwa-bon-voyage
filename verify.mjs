...
try {
  ...
  assert.equal(await page.locator('.tide-timeline-item').count(), 22)
  ...
  assert.equal(await page.locator('.tide-chrono-item').count(), 43)
  ...
  assert.equal(await page.locator('.tide-chrono-item').count(), 15)
  await closeModule(page)

  await page.getByRole('link', { name: '群像', exact: true }).click()
  await page.locator('.view-overlay.show').waitFor()
  await page.waitForURL(/\/relation\.html$/)
  assert.equal(await page.locator('.bcard').count(), 109)
  assert.equal(await page.locator('.bcard[role="button"][tabindex="0"]').count(), 109)
